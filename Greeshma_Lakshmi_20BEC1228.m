% Kopparapu Greeshma Lakshmi
% 20BEC1228

% Project Title: Brain MRI Classification using DWT & SVM
close all
clc
clear all
...
svmStruct1_new = fitcsvm(data1,group,'KernelFunction', 'linear');
species_Linear_new = predict(svmStruct1_new,newfeat);
...
for i = 1:length(label)
    ...
    svmStruct = fitcsvm(xdata(train,:),group(train),'BoxConstraint',Inf,'KernelFunction','rbf');
    classes = predict(svmStruct,xdata(test,:));
    %class = svmclassify(meas(test,:),meas(train,:),label(train,:));
    classperf(cp,classes,test);
end
%Accu = cp.ClassifiedRate;
Accuracy = cp.CorrectRate;
sprintf('Accuracy of classification with 5 fold cross validation is: %g%%',Accuracy*100)
